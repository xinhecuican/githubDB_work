...
#PerORC Model with Molecular Descriptors (Model #4)
...
list1 = glob.glob("ORC PLB IDM 2013/train_graphs/*.npy")
#print(list1[0][21:25])
for i in range(len(list1)):
    list1[i] = list1[i][21:25]

rem = sorted(set(list2)-set(list1))
if len(rem) == 0:
    print("All training data created. No leftovers!")
else:
    for r in rem:
        print(r, int(np.argwhere(list2==r)))

test_list = np.load("ORC PLB IDM 2013/test_list.npz", allow_pickle=True)["arr_0"]
list1 = glob.glob("ORC PLB IDM 2013/test_graphs/*.npy")
#print(list1[0][21:25])
for i in range(len(list1)):
    list1[i] = list1[i][20:24]

rem = sorted(set(test_list)-set(list1))
if len(rem) == 0:
    print("All test data created. No leftovers!")
else:
    for r in rem:
        print(r, int(np.argwhere(test_list==r)))
"""
...
cnt = 0
for t in test_list:
    temp = np.load("ORC PLB IDM 2013/test_graphs/"+t+"_10_15.npy", allow_pickle=True)
    ...
    for i in range(len(temp)):
        ...
        for j in range(0, 10*fp):
            ...
            for (v,w) in temp[i][j].edges():
                try:
                    edges.append(temp[i][j].edges[(v,w)]["ricciCurvature"])
                except:
                    edges.append(0)
            #print(edges)
    ...
    cnt += 1
np.save("ORC PLB IDM 2013/test_set_"+str(fp)+"_model_4.npy", test_set)
...
cnt = 0
for t in train_list:
    temp = np.load("ORC PLB IDM 2013/train_graphs/"+t+"_10_15.npy", allow_pickle=True)
    print("Processing: ", t, cnt)
    train_features = []
    for i in range(len(temp)):
        temp_feature = []
    ...
    cnt += 1
np.save("ORC PLB IDM 2013/train_set_"+str(fp)+"_model_4.npy", train_set)

"""
X_train = np.load("ORC PLB IDM 2013/train_set_"+str(fp)+"_model_4.npy", allow_pickle=True)
X_test = np.load("ORC PLB IDM 2013/test_set_"+str(fp)+"_model_4.npy", allow_pickle=True)
...
np.save("ORC PLB IDM 2013/results_"+str(fp)+"_model_4.npy", [results, gbt_values, rf_values])
"""